...
// For client-side requests (browser), use NEXT_PUBLIC_API_URL
// For server-side requests (SSR in Docker), use API_URL which points to the Docker network
const getApiUrl = () => {
  // Server-side: use internal Docker network URL if available
  if (typeof window === 'undefined') {
    return process.env.API_URL || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1';
  }
